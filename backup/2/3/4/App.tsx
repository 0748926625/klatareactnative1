...
import { StyleSheet, View, Text, Animated, PanResponder, Dimensions, TouchableOpacity, ImageBackground } from 'react-native';
...
export default function App() {
  ...
  useEffect(() => {
    ...
    const subscription = Dimensions.addEventListener('change', ({ window }) => {
      setScreen(window);
      setGroundY(window.height - GROUND_HEIGHT - PLAYER_RADIUS * 2);
    });
  ...
  useEffect(() => {
    // Animation simple du ciel (défilement horizontal)
    const interval = setInterval(() => {
      setSkyOffset(offset => (offset - 1) % screen.width);
